...
def recruit_pos_check(game_map, flag, w, h):
    if flag:
...
def is_invalid_input_move(this_map, pos, w, h, flag):
    ...
    army_one_step_away = [[y1 - 1, x1], [y1 + 1, x1], [y1, x1 - 1],
                          [y1, x1 + 1]]  # positions that one step away from the army
...
def main():
    ...
    while play_again:  # main loop, keep the game going turn by turn
        ...
        while True:
            ...
            if not recruit_pos_check(game_map, player_flag, width, height):
                    ...
                    # Valid input and enough resource, recruit the desired army
                        ...
                        if player_flag:
                            ...
                            if rec_type == "S":
                                ...
                                print("")
                                print("[Your Asset: Wood-{r1} Food-{r2} Gold-{r3}]".format(
                                    r1=players_resources["one"]["W"],
                                    r2=players_resources["one"]["F"],
                                    r3=players_resources["one"]["G"]))
                                break
            ...
            # d.iii Edge cases
            elif rec_type.upper() == "NO":
                break

            elif rec_type.upper() == "QUIT":
                sys.exit("Game terminated")

            elif rec_type.upper() == "DIS":
                display_map(game_map)

            elif rec_type.upper() == "PRIS":
                display_prices()
        ...
        while True:
            ...
            if units_move_check(player_flag, game_map, armies_to_move, width, height):
                ...
                if move_cord.upper() == "QUIT":
                    sys.exit("Game terminated")

                elif move_cord.upper() == "DIS":
                    display_map(game_map)

                elif move_cord.upper() == "PRIS":
                    display_prices()

                elif move_cord.upper() == "NO":
                    break
                    ...
                    # when destination was enemy armies
                        ...
                        if start_label == "T":
                            ...
                            if end_label == "T":
                                ...
                                print(
                                    "Great! We defeated the enemy {} with massive loss!".format(army_names[end_label]))
                                continue
                            ...
                            # when T reaches enemy base
                                ...
                                print(" ")
                                print("***Congratulation! Emperor {a} unified the country in {b}.***".format(
                                    a=commander_name, b=years))
                        ...
                        # encounters home base of other player
                            ...
                            print(" ")
                            print(
                                "***Congratulation! Emperor {a} unified the country in {b}.***".format(a=commander_name,
                                                                                                       b=years))
                            sys.exit()
